...
# Ensure working directory is always the repo root (fixes Streamlit Cloud path issues)
os.chdir(Path(__file__).parent)
sys.path.insert(0, str(Path(__file__).parent / "src"))

from db import get_engine, read_trials, trial_count, last_updated, ensure_schema
...
def no_data_msg(msg: str = "No data matches the current filters.") -> None:
    st.info(f"ℹ️ {msg}")
...
def load_data() -> pd.DataFrame | None:
    """
    Load trial data. Priority:
    1. Remote/local database (Supabase in prod, SQLite locally after ETL)
    2. CSV fallback for local dev convenience

    Returns None only if both sources are empty/missing.
    """
    try:
        engine = get_engine()
        ensure_schema(engine)
        count = trial_count(engine)
        if count > 0:
            df = read_trials(engine)
            if "status_label" not in df.columns and "status" in df.columns:
                df["status_label"] = df["status"].apply(label_status)
            return df
    except Exception as e:
        st.sidebar.warning(f"DB unavailable: {e}")
...
def run_pipeline() -> bool:
    """Run fetch + clean pipeline inline (no subprocess). Returns True on success."""
    try:
        from fetch import fetch_all

        with st.spinner("Fetching from ClinicalTrials.gov... (~3–5 min)"):
            fetch_all(output_path="data/raw_trials.csv")
    except Exception as e:
    ...
    try:
        ...
        with st.spinner("Cleaning and classifying data..."):
            clean(input_path="data/raw_trials.csv", output_path="data/clean_trials.csv")
    except Exception as e:
...
def stop_with_no_data() -> NoReturn:
    """Render the no-data screen and stop execution."""
    ...
    with col_b:
        if st.button("🧪 Load sample data instead"):
            spec = importlib.util.spec_from_file_location(
                "gen", Path(__file__).parent / "generate_sample.py"
            )
            if spec is None or spec.loader is None:
                st.error("Could not load generate_sample.py")
                st.stop()
...
# Extra explicit narrowing for static analysis tools
...
with st.sidebar:
    ...
    min_year = (
        int(df_raw["start_year"].min())
        if not df_raw["start_year"].isna().all()
        else 2010
    )
    max_year = (
        int(df_raw["start_year"].max())
        if not df_raw["start_year"].isna().all()
        else 2025
    )
    ...
    default_labels = [
        label_status(s)
        for s in ["RECRUITING", "ACTIVE_NOT_RECRUITING", "NOT_YET_RECRUITING", "COMPLETED"]
        if label_status(s) in status_label_options
    ]
    selected_status_labels = st.multiselect(
        "Status", status_label_options, default=default_labels
    )
    selected_statuses = [STATUS_LABELS_INV.get(label, label) for label in selected_status_labels]
...
try:
    engine = get_engine()
    _ts = last_updated(engine)
    _extracted = _ts[:10] if _ts else "unknown"
except Exception:
    _extracted = Path(CLEAN_PATH).stat().st_mtime if Path(CLEAN_PATH).exists() else "unknown"
    if isinstance(_extracted, float):
        _extracted = datetime.datetime.fromtimestamp(_extracted).strftime("%Y-%m-%d")
...
def _compare_content(df: pd.DataFrame) -> None:
    ...
    st.caption(
        "Trials are not mutually exclusive — a multinational trial appears in every region where it has sites."
    )
    ...
    with col_phase:
        ...
        if phase_df.empty:
            ...
            phase_df["Phase"] = pd.Categorical(
                phase_df["Phase"], categories=phase_order, ordered=True
            )
...
with tab_overview:
    ...
    with col_a:
        ...
        cancer_counts = (
            df["cancer_type"].value_counts().reset_index().rename(
                columns={"cancer_type": "Cancer Type", "count": "Trials"}
            ).head(15)
        )
    ...
    with col_spon:
        ...
        sponsor_col = "sponsor_parent" if "sponsor_parent" in df.columns else "sponsor_name"
        top_sponsors = df[sponsor_col].value_counts().reset_index().head(15)
        ...
        if top_sponsors.empty:
            ...
            sponsor_origin_map = (
                df.groupby(sponsor_col)["sponsor_origin"]
    ...
    china_recent = df[
        (df["start_year"] >= 2020) & (df["sponsor_origin"] == "Chinese Biotech/Pharma")
    ]
    mnc_recent = df[
        (df["start_year"] >= 2020) & (df["sponsor_origin"] == "MNC")
    ]
    ...
    with st.expander("📋 Browse Raw Data"):
        ...
        col_labels = {
            ...
            "phase_clean": "Phase",
            "modality": "Modality",
            "sponsor_name": "Sponsor (original)",
            "sponsor_parent": "Sponsor (consolidated)",
            "sponsor_origin": "Sponsor Origin",
            "status_label": "Status",
            "start_year": "Start Year",
            "is_multinational": "Multinational",
